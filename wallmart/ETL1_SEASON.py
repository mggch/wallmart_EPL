...
unpickled_df = pd.read_pickle("wallmart/pkl/season_"+sys.argv[1]+".pkl")
...
if not df_E0.empty:
    ...
    df_E0['SCORE'] = df_E0.apply(lambda x: get_score(x['FTR']), axis=1)

    df_HomeTeam = df_E0.groupby('HomeTeam')['SCORE'].sum().reset_index()
    df_HomeTeam.set_index('HomeTeam')
    df_HomeTeam.rename(columns={'HomeTeam': 'Team'},inplace=True)

    df_AwayTeam = df_E0.groupby('AwayTeam')['SCORE'].sum().reset_index()
    df_AwayTeam.set_index('AwayTeam')
    df_AwayTeam.rename(columns={'AwayTeam': 'Team'},inplace=True)

    df_SCORE = df_HomeTeam.append(df_AwayTeam)
    df_SCORE = df_SCORE.groupby('Team')['SCORE'].sum().reset_index()

    df_SCORE = df_SCORE.sort_values(by=['SCORE'], ascending=False)

    df_SCORE.to_csv("wallmart/report/E0_TABLAPOSICION_"+sys.argv[1]+".csv", index = False, header=True)
...
if not df_PL.empty:
    ...
    df_PL['SCORE'] = df_PL.apply(lambda x: get_score(x['FTR']), axis=1)
    ...
    df_SCORE.to_csv("wallmart/report/PL_TABLAPOSICION_"+sys.argv[1]+".csv", index = False, header=True)
...
if not df_E0.empty:
    ...
    df_HomeTeam_HST.set_index('HomeTeam')
    df_HomeTeam_HST.rename(columns={'HomeTeam': 'Team'},inplace=True)

    df_HomeTeam_FTHG = df_E0.groupby('HomeTeam')['FTHG'].sum().reset_index()     # Goles Marcados 
    df_HomeTeam_FTHG.set_index('HomeTeam')
    df_HomeTeam_FTHG.rename(columns={'HomeTeam': 'Team'},inplace=True)
    ...
    df_AwayTeam_AST.set_index('AwayTeam')
    df_AwayTeam_AST.rename(columns={'AwayTeam': 'Team'},inplace=True)
    
    df_AwayTeam_FTAG = df_E0.groupby('AwayTeam')['FTAG'].sum().reset_index()     # Goles Marcados 
    df_AwayTeam_FTAG.set_index('AwayTeam')
    df_AwayTeam_FTAG.rename(columns={'AwayTeam': 'Team'},inplace=True)
    ...
    df_INDGOL = df_INDGOL.sort_values(by=['INDGOL'], ascending=False)
    
    df_INDGOL.to_csv("wallmart/report/E0_INDGOL_"+sys.argv[1]+".csv", index = False, header=True)
...
if not df_PL.empty:
    ...
    df_INDGOL.to_csv("wallmart/report/PL_INDGOL_"+sys.argv[1]+".csv", index = False, header=True)   
...
if not df_E0.empty:
    ...
    df_HomeTeam_FTAG.set_index('HomeTeam')
    df_HomeTeam_FTAG.rename(columns={'HomeTeam': 'Team'},inplace=True)  
        
    df_AwayTeam_FTHG = df_E0.groupby('AwayTeam')['FTHG'].sum().reset_index()     # Goles Marcados Contra
    df_AwayTeam_FTHG.set_index('AwayTeam')
    df_AwayTeam_FTHG.rename(columns={'AwayTeam': 'Team'},inplace=True)
    ...
    df_CANGOLCONTRA = df_CANGOLCONTRA.sort_values(by=['CANGOLCONTRA'], ascending=False)
    
    df_CANGOLCONTRA.to_csv("wallmart/report/E0_CANGOLCONTRA_"+sys.argv[1]+".csv", index = False, header=True)
...
if not df_PL.empty:
    ...
    df_CANGOLCONTRA.to_csv("wallmart/report/PL_CANGOLCONTRA_"+sys.argv[1]+".csv", index = False, header=True)
